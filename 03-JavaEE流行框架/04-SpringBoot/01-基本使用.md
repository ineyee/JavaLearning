...
SpringBoot 就是为解决这些问题而生的框架，它已经内置了 Spring 和 SpringMVC 这两个框架并能自动添加依赖和配置，只剩 MyBatis 需要我们手动整合，但整合的方式也极其简单，不需要再像以前一样添加一大堆的依赖 + 编写一大堆的配置。换句话说，以前我们是用 SSM 开发项目，现在是用 SpringBoot + MyBatis 开发项目，SpringBoot 之所以内置 Spring 和 SpringMVC 这两个框架而不内置 MyBatis，是因为 Spring 和 SpringMVC 本来就是 Spring 官方体系里的两个框架、SpringBoot 想怎么内置就怎么内置，而 MyBatis 则不是 Spring 官方体系里的框架、所以不能内置。并且 SpringBoot 还内置了 Tomcat，这就使得项目可以直接运行，我们都不用像以前那样在本机自己安装 Tomcat 了。

## 二、怎么使用 SpringBoot

> 本篇只是初步体验，把 SpringBoot 给跑通，SpringBoot 的详细使用见后面 SpringBoot + MyBatis
