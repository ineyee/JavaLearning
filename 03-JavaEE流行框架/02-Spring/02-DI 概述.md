...
我们手动写代码给对象的成员变量赋值时，可以通过 setter 方法赋值，也可以通过构造方法赋值，这里的通过 setter 方法注入其实就对应通过 setter 方法赋值，比较常用
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <bean id="person3" class="com.ineyee._03_dibysetter.domain.Person">
        ...
        <property name="dog">
            <!--
                当 bean 标签写在 property 标签里面时，其实类似于局部对象、框架会创建多个，所以就不用写 id 以供获取了，因为这个局部对象只属于当前对象的属性
                当某个对象并非全局只需要创建一个时，就可以把它的 bean 标签放在 property 标签里
    ...
    <!--
        当 bean 标签写在最外层时，框架其实只会全局创建一个该对象，也就是一个单例对象，其它任意地方通过 id 获取到的都是这同一个单例对象
        当某个对象全局只需要创建一个时，就可以把它的 bean 标签放在最外层
...
我们手动写代码给对象的成员变量赋值时，可以通过 setter 方法赋值，也可以通过构造方法赋值，这里的通过构造方法注入其实就对应通过构造方法赋值，不太常用
