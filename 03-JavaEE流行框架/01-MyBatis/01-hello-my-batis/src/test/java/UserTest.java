...
//            UserDao userDao = session.getMapper(UserDao.class);
            ...
            List<UserBean> userBeanList = userDao.list(Map.of(
                    "limit", pageSize,
...
//            UserDao userDao = session.getMapper(UserDao.class);
            ...
            Integer pageNum = 1; // 当前页码，从 1 开始
            PageHelper.startPage(pageNum, pageSize);
            List<UserBean> userBeanList = userDao.listPageHelper();
            if (!userBeanList.isEmpty()) {
                System.out.println("查询成功：" + userBeanList);
            } else {
                System.out.println("查询失败");
...
//        }
        ...
        try (SqlSession session = MyBatisUtil.openSession()) {
            ...
            List<Integer> idList = List.of(46, 47, 48);
...
//        }
    ...
    // 批量更新成功一般是给客户端返回成功信息、因为客户端本来就知道更新的是哪批数据、哪些字段，批量更新失败一般是给客户端返回失败信息
    @Test
    void updateBatch() {
//        // 创建一个会话，项目运行期间可以创建多个
//        try (SqlSession session = MyBatisUtil.openSession()) {
...
//        }
        ...
        try (SqlSession session = MyBatisUtil.openSession()) {
            ...
            int ret = userDao.updateBatch(Map.of(
                    "idList", List.of(43, 44, 45),
                    "height", 2.03
            ));
            if (ret > 0) {
