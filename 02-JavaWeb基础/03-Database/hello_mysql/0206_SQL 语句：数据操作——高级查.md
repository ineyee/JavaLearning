...
-- 查询表里所有苹果手机评分字段的平均值
SELECT AVG(score) AS appleAvgScore
FROM t_product
WHERE brand = '苹果';
...
-- 查询表里所有苹果手机评分字段的最小值
SELECT MIN(score) AS appleLowestScore
FROM t_product
WHERE brand = '苹果';
...
SELECT
	brand,
	COUNT(*) AS appleCount,
	MAX(price) AS appleMaxPrice,
	AVG(score) AS appleAgvScore 
FROM t_product
WHERE brand = '苹果';
...
SELECT
	brand,
	COUNT(*) AS hwCount,
	MAX(price) AS hwMaxPrice,
	AVG(score) AS hwAgvScore 
FROM t_product 
WHERE brand = '华为';
...
SELECT
	brand,
	COUNT(*) AS miCount,
	MAX(price) AS AS miMaxPrice,
	AVG(score) AS miAgvScore 
FROM t_product 
WHERE brand = '小米';
```
...
SELECT
	...
	AVG(score) AS agvScore 
FROM t_product 
GROUP BY brand HAVING maxPrice <= 8000;
```
