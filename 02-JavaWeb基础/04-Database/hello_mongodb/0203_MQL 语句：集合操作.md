...
添加索引固然能**提高数据查询效率**，但是添加索引后每一次对数据的增删改操作都会影响索引树的变化，也就是说索引会**降低数据写入效率**。所以**一定要根据实际业务场景的查询条件来决定用哪些字段做索引**，千万不要用每个字段都做索引。
...
那么在**产品详情界面的查询条件**，无非就是根据 id 查询一下 deleted == 0 的某条数据。所以我们可能会想创建一个 { "_id": 1, "deleted": 1 } 的复合字段索引，但其实这没必要，因为每个集合默认都有一个 \_id 单字段索引，而用 \_id 字段肯定已经能直接精准定位到某条数据了，定位到某条数据后在内存里再判断一下这条数据的 deleted 是否等于 0、这个过滤成本可以说就是 0、根本不会增加时间复杂度，所以就算我们创建了那个复合字段索引，在查询时 MongoDB 优化器也会优先匹配使用 \_id 单字段索引，毕竟两个索引的时间复杂度一样、但是复合索引明显更复杂。**因此在有 \_id 字段的查询里，我们一律不需要用 \_id 和其它字段构建复合字段索引，直接用默认的 _id 单字段索引即可。**

那么在**产品列表界面的查询条件**，比如我们业务上设计可以通过 name 查询、与此同时要查询 deleted == 0 的某些数据，此时我们就可以创建一个 { "deleted": 1, "name": 1 } 的复合字段索引；又比如我们业务上设计可以通过 desc 查询、与此同时要查询 deleted == 0 的某些数据，此时我们就可以创建一个 { "deleted": 1, "desc": 1 } 的复合字段索引。你可能会想我们可不可以直接添加一个 { "deleted": 1, "name": 1, "desc": 1 } 的复合字段索引，可以是可以，但是这样大的索引做可能会带来更多的副作用。记住**添加索引是为了高频查询场景的查询条件，不是为了给字段添加的，**所以如果我们业务上设计就是多条件一起查是高频场景，那就大索引没问题，否则就小索引。

此外**产品列表界面的排序条件一般都需要做索引**，比如我们业务上设计可以通过 price 排序、与此同时要查询 deleted == 0 的某些数据，此时我们就可以创建一个 { "deleted": 1, "price": 1 } 的复合字段索引。

**并且因为复合字段索引遵循最左索引字段原则，所以为了内存和性能更优考虑，我们应该删掉那些与复合字段索引第一个字段相同的单字段索引。**
