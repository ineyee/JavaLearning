...
* 下载完双击解压即可，解压后把解压产物（如 nginx-1.28.2 这个文件夹）直接拖到 /usr/local 目录下（即 /usr/local/nginx-1.28.2），然后需要手动编译安装（/ 代表根目录 Macintosh HD，~/ 代表当前用户目录 /Users/ineyee）
  ...
  * 然后在 nginx 目录下手动创建一个 logs 目录（即 /usr/local/nginx-1.28.2/logs），用来存放 HTTP 访问日志和 Nginx 错误日志
...
* 打开 nginx 目录下的 nginx.conf 配置文件（即 /usr/local/nginx-1.28.2/conf/nginx.conf），确认配置项
  ...
  user  nginx;
  # 用来指定 worker 进程数，根据当前系统的 CPU 核数自动设定
  worker_processes auto;
  
  # Nginx 错误日志的路径及级别（生产环境需要修改路径）
  error_log logs/error.log notice;
  # pid 的存放文件为 run/nginx.pid，方便进行进程管理（生产环境需要修改路径）
  pid run/nginx.pid;
  
  events {
      # 用来指定一个 worker 进程同一时间最多能处理多少个请求
      worker_connections  1024;
  }
  
  http {
    	# 如果觉得一个配置文件里的内容太多，可以拆分成多个小配置文件，然后用 include 来导入
    	# 它的值可以是其它配置文件的相对路径或绝对路径
      include       mime.types;
      ...
      # HTTP 请求的日志的存放路径为 logs/access.log 这个路径，日志的格式为 main（生产环境需要修改路径）
