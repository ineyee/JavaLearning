...
假设我们只有一台 Nginx 服务器，那么一旦这台服务器挂掉，整个系统就运转不起来了 ，这就是单点故障问题。所以我们可以**搞一台 Nginx 主服务器、再搞一台 Nginx 备服务器，这就是主备模式。正常情况下流量全部走主服务器，备服务器处于待命状态。Keepalived 会持续检测主服务器的健康状态，一旦主服务器挂掉，Keepalived 就会将虚拟 IP（VIP）自动漂移到备服务器上，备服务器接管流量，之前的主服务器如果重启了就重新变回主服务器，**成功避免单点故障问题。但主备模式有一个缺点：备服务器平时处于空闲状态，资源浪费。

#### 2、多主服务器 + 负载均衡

主备模式能避免单点故障问题，但是备服务器平时闲置，也无法提高吞吐量。所以我们可以**搞成多台 Nginx 同时对外提供服务，在它们前面再加一层负载均衡（如 LVS 或云厂商的 SLB），将流量分发到多台 Nginx 上，**这样每台 Nginx 都在工作，成功解决资源浪费和提高吞吐量的问题。
...
LVS / SLB（四层负载均衡）
    ↓          ↓
 Nginx-1    Nginx-2    （七层，处理 HTTP、限流、缓存等）
    ↓          ↓
        Tomcat 集群
```
...
* 多台 Nginx 同时工作，任意一台挂掉不影响整体服务，天然避免单点故障
* 可以水平扩展，流量增大时直接加机器