...
但是我们项目里 resources 目录下的众多配置文件也都打包进 runnable jar 包里了，线上如果要修改的话，怎么修改呢？不可能改 Java 代码、重新打包吧（mapper 文件 ~= Java 代码等级，几乎不会在线上直接改，所以改 mapper = 改逻辑 = 重新发版，这里我们就不用操心 mapper 了）

其实 SpringBoot 会优先加载 runnable jar 包同级 config/ 目录下的配置文件（只会找 yml 配置文件，不会找日志配置文件），找不到的话才会加载 runnable jar 包里面的配置文件，所以我们可以把线上需要修改的配置文件搞一份到服务器上。**① 去 application.yml 配置文件里确认一下激活的是不是线上环境 prd；② 去云服务器的网络和安全组里确认一下有没有添加 application-prd.yml 配置文件里生产环境下的端口号**

<img src="img/image-20260205215345866.png" alt="image-20260205215345866" style="zoom:50%;" />

#### 4、运行我们的项目

###### 4.1 方式一：直接前台运行（不推荐生产环境）
...
优点：简单直接，适合调试，可以直接看到日志输出

缺点：无法后台运行，终端关闭后进程会终止

###### 4.2 方式二： nohup 后台运行（常用）
...
* ② 在 bin 目录下创建一个启动脚本文件 start.sh，cd /usr/local/soft/${finalName}、执行 bin/start.sh 即可，使用场景：
  ...
  * 项目第一次部署到服务器后，上传 jar 包到服务器，启动应用
  * 服务器重启后，应用不会自动启动，需要手动启动
  * 应用崩溃退出后，重新启动
  ...
  # 应用配置，应用变量定义区
  APP_HOME=/usr/local/soft/tp # 应用根目录（换应用需要改）
  JAR_FILE=$APP_HOME/tp.jar # 应用 runnable jar 包（换应用需要改）
  LOG_CONFIG=$APP_HOME/config/logback-spring.xml # 应用日志配置文件
  PID_FILE=$APP_HOME/bin/app.pid # 保存 Java 进程 PID 的文件，这个启动脚本会自动生成 app.pid 文件，并在应用启动后把 PID 存进去
  
  # 检查应用是否已经在运行
  # 如果 PID 文件已存在，代表曾经运行过，那就读取出 PID
  if [ -f $PID_FILE ]; then
      PID=$(cat $PID_FILE)
      # 如果应用的确正在运行，那就退出执行脚本，避免启动多个应用
...
* ④ 在 bin 目录下创建一个重启脚本文件 restart.sh，cd /usr/local/soft/${finalName}、执行 bin/restart.sh 即可，使用场景：
  ...
  * 修改了 application.yml、application-prd.yml 或 logback-spring.xml 配置文件后，重启应用使配置生效
  * 发布新版本，上传新 jar 包，重启应用（一步到位，自动停止旧版本，启动新版本）
...
* ⑤ 在 bin 目录下创建一个查看应用运行状态脚本文件 status.sh，cd /usr/local/soft/${finalName}、执行 bin/status.sh 即可
