...
```shell
# SpringBoot 只会找 yml 配置文件，不会找日志配置文件，所以运行时得指定日志配置文件的路径
...
```shell
# nohup：No Hang Up，关闭终端，进程继续跑
# &：后台运行
nohup java -jar tp.jar --logging.config=file:/usr/local/soft/tp/config/logback-spring.xml &
```
