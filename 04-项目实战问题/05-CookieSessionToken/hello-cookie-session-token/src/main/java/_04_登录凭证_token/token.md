...
因此我们需要处理这个分布式部署下的 session 共享问题，解决的办法就是：客户端要登录，这一次 Nginx 负载均衡服务器把这个登录请求转发给了服务器 3，那登录成功后 session 对象不存储在服务器 3 上，而是存储在 Redis 分布式缓存服务器上；客户端下次要支付、携带了 session，这一次 Nginx 负载均衡服务器把这个登录请求转发给了服务器 1，但是不要在服务器 1 上获取 session 会话对象，而是去 Redis 分布式缓存服务器上获取，这样一来就能正常获取到了，可以正常执行后续的操作。这样的确可以解决问题，但是多了一层 Redis 访问、性能下降，系统复杂度也上升了很多，Redis 也成了单点、一旦 Redis 故障、就算应用服务器都 ok、所有业务也都无法正常运转
