...
* 下载完双击解压即可，解压后把解压产物（如 redis-8.6.1 这个文件夹）直接拖到 /usr/local 目录下（即 /usr/local/redis-8.6.1），然后需要手动编译安装（/ 代表根目录 Macintosh HD，~/ 代表当前用户目录 /Users/ineyee）
  ...
  * 然后打开 redis 目录下的 redis.conf 配置文件（即 /usr/local/redis-8.6.1/redis.conf），确认配置项
    ...
    # 1️⃣ 网络相关配置
    # -------------------------------
    bind 127.0.0.1    # 只允许本机访问（生产环境需要修改）
    port 6379         # 监听端口（默认 6379）
    
    # -------------------------------
    # 2️⃣ 通用配置
    # -------------------------------
    daemonize no                                      # 是否后台运行（no=前台运行，yes=后台运行）
    logfile /usr/local/redis-8.6.1/log/redis.log      # 日志文件路径（生产环境需要修改）
    databases 16                                      # 数据库数量，默认 16 个
    
    # -------------------------------
    # 3️⃣ 持久化相关配置
    # -------------------------------
    dir /usr/local/redis-8.6.1/data    # 数据库存放路径（生产环境需要修改）
    # RDB 持久化配置
    save 900 1       # 900 秒内至少有 1 个 key 被修改则触发 RDB 持久化
    save 300 10      # 300 秒内至少有 10 个 key 被修改则触发 RDB 持久化
    save 60 10000    # 60 秒内至少有 10000 个 key 被修改则触发 RDB 持久化
    # AOF 持久化配置
    appendonly yes              # 是否开启 AOF 持久化（yes=开启，no=关闭）
    appendfilename "appendonly.aof"    # AOF 文件名
    appendfsync everysec        # AOF 同步策略（always=每次写入都同步，everysec=每秒同步一次，no=由操作系统决定）
    
    # -------------------------------
    # 4️⃣ 安全相关配置
    # -------------------------------
    requirepass Redis666!    # 设置访问密码（生产环境必须设置，必须创建用户后才能使用）
    
    # -------------------------------
    # 5️⃣ 内存管理
    # -------------------------------
    maxmemory 256mb                  # 最大内存限制（生产环境需要根据实际情况调整）
    maxmemory-policy allkeys-lru     # 内存淘汰策略（allkeys-lru=所有 key 中淘汰最近最少使用的）
    ```
