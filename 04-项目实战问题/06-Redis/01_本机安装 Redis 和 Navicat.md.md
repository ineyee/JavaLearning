...
* 下载完双击解压即可，解压后把解压产物（如 redis-8.6.1 这个文件夹）直接拖到 /usr/local 目录下（即 /usr/local/redis-8.6.1），然后需要手动编译安装（/ 代表根目录 Macintosh HD，~/ 代表当前用户目录 /Users/ineyee）
  ...
  * 然后打开 redis 目录下的 redis.conf 配置文件（即 /usr/local/redis-8.6.1/redis.conf），确认配置项
    ...
    # RDB 持久化配置
    # RDB 文件的名字，文件的路径：/usr/local/redis-8.6.1/data/dump.rdb
    dbfilename "dump.rdb"
    # RDB 快照生成策略
    ...
    appendonly yes
    # AOF 目录名，目录的路径：/usr/local/redis-8.6.1/data/appendonlydir
    # 我们只要配置目录即可，日志文件由 Redis 自动维护
    appenddirname "appendonlydir"
    # AOF 追加命令策略（always = 每次写入都同步，everysec = 每秒同步一次，no = 由操作系统决定）
