...
for (int i = 0; i < orderStatusList.length; i++) {
  ...
  if (orderStatus == 1) {
    // 待支付
    label.text = "待支付";
  } else if () if (orderStatus == 2) {
    // 已支付
    label.text = "已支付";
  } else if () if (orderStatus == 3) {
    // 已发货
    label.text = "已发货";
  } else if () if (orderStatus == 4) {
    // 已完成
    label.text = "已完成";
  } else if () if (orderStatus == 5) {
    // 已取消
    label.text = "已取消";
  }
}
...
> 数据字典几乎是后台管理系统的标配，因为后台管理系统里一般都有大量枚举类型的选项让用户配置，实际开发中我们一般都是先把这些数据字典的表给创建好，并把数据录入到数据库里