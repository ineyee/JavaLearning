...
public class Main {
	...
	public static void test2() {
		 ...
		 * 异常的分类：
		 * 1、检查型异常
		 * 所谓检查型异常是指编译器会检查我们开发者有没有通过try-catch或者throws处理的异常，具体地说如果开发者没有处理这类异常，编译器直接报错、编译根本不通过
		 * 检查型异常，就算你写的代码再规范再健壮也无法100%避免、我们必须得通过try-catch或者throws处理，下面我们会举一些经典的检查型异常：test10、test11。
		 * 上面除了RuntimeException和Error以外的异常都是检查型异常。
		 * 
		 * 2、非检查型异常
		 * 所谓非检查型异常是指编译器不会检查我们开发者有没有通过try-catch或者throws处理的异常，具体地说即便开发者没有处理这里异常，编译器也不会报错的异常
