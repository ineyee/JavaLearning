...
public class Main {
	...
	public static void test2() throws IOException {
		...
		// 用相对路径创建的一个文件夹对象（会从项目的根目录开始找，最前面不用加/或\\）
		File file3 = new File("res");
		// 用相对路径创建的一个文件对象
		File file4 = new File("res/1.txt");
		
		// 创建文件夹（不存在才会创建，存在则不会创建）
		file1.mkdir(); // 只会创建当前文件夹
		file3.mkdirs(); // 除了会创建当前文件夹外，如果父文件夹不存在还会帮忙创建好父文件夹，比mkdir牛逼点
		
		// 创建文件（不存在才会创建，存在则不会创建）
		file2.createNewFile();
		file4.createNewFile();
...
//		System.out.println(file3.getAbsolutePath()); // /Users/yiyi/Desktop/JavaProject/06-IO/res
//		System.out.println(file4.getAbsolutePath()); // /Users/yiyi/Desktop/JavaProject/06-IO/res/1.txt
		
		// 4、获取文件或文件夹的绝对路径对应的File对象
...
//		System.out.println(file3.getAbsoluteFile()); // /Users/yiyi/Desktop/JavaProject/06-IO/res
//		System.out.println(file4.getAbsoluteFile()); // /Users/yiyi/Desktop/JavaProject/06-IO/res/1.txt
...
//		System.out.println(file4.getParentFile()); // /users/yiyi/desktop
		...
		// 8、获取文件的大小，单位为字节bytes（注意文件夹没有大小，计算文件夹的大小只能遍历文件然后累加）
//		System.out.println(file1.length()); // 96
...
//		System.out.println(file4.canWrite()); // true
		...
		// 12、获取当前文件夹里所有的文件和文件夹（不会去子文件夹里找），还可以过滤
//		File file = new File("/Users/yiyi/Desktop/JavaProject/06-IO");
...
//		System.out.println(Arrays.toString(filterFiles)); 
		...
		// 13、删除文件或空文件夹（如果文件夹里有东西则不会删除，注意不会放到回收站，直接就抹掉了，所以要慎重）
		file2.delete();
	}
