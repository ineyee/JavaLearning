...
interface Runnable {
	// 用 abstract 修饰的方法，外界必须实现
	abstract void slowRun();
	
	/*
	 * 用 default 修饰的方法，外界不必须实现
	 * 
	 * 接口升级问题
	 * 
	 * 所谓接口升级就是给接口增加新的方法。
	 * 
	 * 实际开发中我们肯定会遇到接口升级问题，比如项目第一期我们的接口里定义了一堆方法，然后有几百个类
 ...
 * Java接口里的方法则都是必须实现的，当然Java8之后提供了default关键字，我们可以在接口里为某个方法提供一个默认的实现，那别的类就可以不必须实现这个方法了，但是default关键字和abstract关键字是不能同时使用的
 * 所以我们可以理解为abstract关键字修饰的方法外界必须实现，default关键字修饰的方法外界可选实现（因为提供了默认实现嘛）
 *
